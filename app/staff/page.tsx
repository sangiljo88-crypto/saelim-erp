...
export default async function StaffPage() {
  ...
  // 실제 직원 명부(DB members)만 표시 — 테스트용 MOCK 계정은 직원 목록에서 제외
  // (시드 완료 후 가상 인물(공장장/팀장/김현수 등)이 실직원과 섞이는 문제 방지)
  const allStaff = (dbMembers ?? []).map(m => ({
    ...m,
    isLegacy: false,
    base_salary: salaryMap[m.login_id] ?? 0,
  }));
